...
const adminNav: NavItem[] = [
  { name: "KYC Queue", href: "/admin" },
  { name: "Escrow", href: "/admin" },
];
...
export default function AdminConsolePage() {
  ...
  return (
    <ProtectedRoute allowedRoles={["ADMIN"]}>
      <PortalShell portal="admin" navItems={adminNav}>
        <div className="space-y-6">
          <div>
            <h1 className="text-3xl font-extrabold text-slate-900">Admin Console</h1>
            <p className="text-sm text-slate-500 mt-1">Verify tutors, release escrow, and manage platform operations.</p>
          </div>
          ...
          {activeTab === "matching" && (
            <EmptyState icon={Sparkles} title="AI Lead Matcher" description="Manual matching tools coming soon. Use the matching API for automated recommendations." />
          )}
          ...
          <Link href="/" className="text-xs font-bold text-indigo-600 hover:underline">Back to Home</Link>
        </div>
      </PortalShell>
    </ProtectedRoute>
  );
